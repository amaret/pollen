...
public class ParseUnit {
	...
	public static final String POLLEN_PREFIX = "pollen.";
	...
	public static final String POLLEN_PREFIX__ = "pollen__";
	public static final String DEFAULT_LOOPVAR = POLLEN_PREFIX__ + "loopvar"; // for loops w/ undeclared loop variables
	public static final String INTRINSIC_UNITVAR = POLLEN_PREFIX__ +"unitname";
	...
	public static final String PRESET_INIT = POLLEN_PREFIX__ + "presets__";
	...
	public static boolean isIntrinsicCall(String s) {
		if (!(s.matches(ParseUnit.POLLEN_PREFIX__ + ".*")))
			return false;
		if (s.equals(ParseUnit.INTRINSIC_UNITVAR))
			return false;
		if (s.equals(ParseUnit.POLLEN_PREFIX__ + ParseUnit.DEFAULT_LOOPVAR))
			return false;			
		return true;
	}
	...
	public boolean putPollenIntrinsicFcn(String fname) {
        ...
        fname = ParseUnit.current().getCurrUnitNode().getPkgName().getText().replace('.', '_') + '_'  + uname + '_'
        		+ ParseUnit.POLLEN_PREFIX__ + fname + ParseUnit.KIND_EXTERN;
			...
			if (!(currNode instanceof ImportNode)) {
				try {
					...
					if (pkgPath == null) {
						...
						boolean isPollenPrefix = fromPkg.matches(ParseUnit.POLLEN_PREFIX + ".*");
						// A pollen intrinsic or protocol. Import generated by parser.
						if (isPollenPrefix) {
							currUnit = parseIntrinsicProtocol(currImport,
									fromPkg, pkgPath, client, importFromClient);
						}
	 ...
	 * @param pkgPath
	 * @param client
	 * @param importFromClient
	 * @return the parsed intrinsic protocol
	 * @throws Exception
	 */
	private UnitNode parseIntrinsicProtocol(ImportNode currImport,
			String fromPkg, String pkgPath, UnitNode client,
			ImportNode importFromClient) throws Exception {
		UnitNode currUnit;
		...
		else { // SleepWake, DynamicMemory, etc.
			currUnit = parseUnit(ProcessUnits.getPollenBundles() + File.separator + fromPkg 
					+ File.separator
					+ currImport.getUnitName().getText()
					+ ".p", client, importFromClient);
		}
